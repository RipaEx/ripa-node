...
for(var i=1; i<52; i++){
  var delegate = {
	...
	//send ripa to delegate
	var premineTx = ripajs.transaction.createTransaction(delegate.address, delegate.balance, null, premine.passphrase);
  ...
  //
	// //vote for itself
	// var voteTransaction = ripajs.vote.createVote(delegate.passphrase,["+"+delegate.publicKey]);
	// voteTransaction.fee = 0;
	// voteTransaction.timestamp = 0;
	// voteTransaction.senderId = delegate.address;
	// voteTransaction.signature = ripajs.crypto.sign(voteTransaction,ripajs.crypto.getKeys(delegate.passphrase));
